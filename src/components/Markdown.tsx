...
export function Heading({ children, level }: any) {
    let size
    switch (level) {
        case 1:
    ...
    return (
        <h1 className={`mt-4 mb-2 font-medium leading-tight ${size}`}>
            {children}
        </h1>
    )
}

export function List({ children, ordered }: any) {
    return ordered ? (
...
export function Code({ language, value }: any) {
    return (
        <Highlight className={`my-4 shadow rounded ${language}`}>
            {value}
        </Highlight>
    )
}

export function InlineCode({ value }: any) {
    return (
...
export function Image({ alt, src }: any) {
